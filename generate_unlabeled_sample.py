...
if __name__ == "__main__":
    L = 80
    #temperatures = np.concatenate((np.arange(1.5,3.2,0.1),np.arange(1.75,3.05,0.1)))  # Temperatures to simulate
    temperatures = np.arange(1.6,2.9,0.1) # Temperatures to simulate
    lattice_type = 'square'
    num_samples = 100
    steps = 100000
    save_dir = "/Users/wolfram/Downloads/ML_Ising/pca_analysis/L_80"
    # save_dir =  "ising_samples_parallel"
    
    # Generate squre samples in parallel for training
    IsingModel.generate_samples_parallel(
        L=L,
